...
import { DOMMapping, SourceLocation } from "./types";
 ...
 */
export async function extractMappings(targetUrl: string): Promise<DOMMapping[]> {
  ...
  try {
    ...
    // ------------------------------------------------------------------
    const rawMappings = await page.evaluate((snippetMaxLen: number) => {
      /**
      ...
      elements.forEach((el) => {
        ...
        const line = parseInt(
          beforeLastColon.substring(secondLastColon + 1),
          10
        );
    ...
    // ------------------------------------------------------------------
    const mappings: DOMMapping[] = rawMappings.map((raw) => ({
